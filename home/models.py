...
class AdvertPlacement(models.Model):
    ...
    advert = models.ForeignKey('home.Advert', on_delete=models.CASCADE, related_name='+')
...
class homeRendition(AbstractRendition):
    image = models.ForeignKey('homeImage', on_delete=models.CASCADE, related_name='renditions')
...
class HomePageHero(Orderable, RelatedLink):
    page = ParentalKey('home.HomePage', related_name='hero')
...
class StandardPageContentBlock(Orderable, ContentBlock):
    page = ParentalKey('home.StandardPage', related_name='content_block')
...
class StandardPageClient(Orderable, RelatedLink):
    page = ParentalKey('home.StandardPage', related_name='clients')
    image = models.ForeignKey(
        'home.homeImage',
        null=True,
        blank=True,
...
class AboutPageRelatedLinkButton(Orderable, RelatedLink):
    page = ParentalKey('home.AboutPage', related_name='related_link_buttons')


class AboutPageOffice(Orderable):
    page = ParentalKey('home.AboutPage', related_name='offices')
...
class AboutPageContentBlock(Orderable):
    page = ParentalKey('home.AboutPage', related_name='content_blocks')
...
class ReasonToJoin(Orderable):
    page = ParentalKey('home.JobIndexPage', related_name='reasons_to_join')
    image = models.ForeignKey(
        'home.homeImage',
        null=True,
        blank=True,
...
class JobIndexPageJob(Orderable):
    page = ParentalKey('home.JobIndexPage', related_name='job')
...
class GoogleAdGrantsPageGrantsManaged(models.Model):
    page = ParentalKey('home.GoogleAdGrantsPage', related_name="grants_managed")
    image = models.ForeignKey(
        'home.homeImage',
...
class GoogleAdGrantsPageQuote(Orderable):
    page = ParentalKey('home.GoogleAdGrantsPage', related_name="quotes")
...
class GoogleAdGrantsAccreditations(Orderable):
    page = ParentalKey('home.GoogleAdGrantsPage', related_name="accreditations")
    image = models.ForeignKey(
        'home.homeImage',
        null=True, blank=True,
        on_delete=models.SET_NULL,
    ...
    def serve(self, request, *args, **kwargs):
        if request.is_ajax() and request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(
                    request,
                    'home/includes/ad_grant_application_landing.html',
                    {'self': self, 'form': form}
                )
            else:
                return render(
...
class ContactLandingPageRelatedLinkButton(Orderable, RelatedLink):
    page = ParentalKey('home.Contact', related_name='related_link_buttons')


@method_decorator(never_cache, name='serve')
class Contact(WagtailCaptchaEmailForm):
    intro = RichTextField(blank=True)
    main_image = models.ForeignKey('home.homeImage', null=True,
                                   blank=True, on_delete=models.SET_NULL,
                                   related_name='+')
    landing_image = models.ForeignKey('home.homeImage', null=True,
                                      blank=True, on_delete=models.SET_NULL,
                                      related_name='+')
...
class MenuItemBlock(StructBlock):
    ...
    subitems = SubMenuItemBlock(required=False)

    class Meta:
        template = "home/includes/menu_item.html"
